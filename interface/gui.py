...
class BlissApp(tk.Frame):
    ...
    def display_menu(self):
        ...
        self.menu_container.grid(
            column=0, row=0, sticky="nsew", columnspan=colspan, rowspan=rowspan
        )

        self.translate_button = tk.Button(
            master=self.menu_container,
            text="translate",
    ...
    def text_box_widget(self, **kwargs):
        ...
        self.word_label = Label(
            master,
            font="arial",
            text=kwargs.get("label_text", "word"),
            width=kwargs.get("label_width", None),
            height=kwargs.get("height", None),
        )
        self.word_label.grid(column=0, row=1)

        self.word_field = Text(
            self.word_label,
            "",
            font="arial",
    ...
    def select_pos_widget(self, **kwargs):
        ...
        self.pos_label = Label(
            master, font="arial", text=label_text, width=kwargs.get("width", None)
        )
        self.pos_label.grid(column=0, row=2)

        pos_names = blisscribe.POS_KEY.values()
        self.pos_field = Listbox(
            self.pos_label,
    ...
    def select_lang_widget(self, **kwargs):
        ...
        self.lang_field = Listbox(
            master=self.lang_label,
            width=len(max(langs, key=len)),
            height=len(langs),
            exportselection=0,
        )
    ...
    def enter_button_widget(self, **kwargs):
        ...
        self.enter_button = tk.Button(
            master=master, text="enter", command=self.render_blissymbol
        )
    ...
    def render_blissymbol(self):
        ...
        if len(word) != 0:
            ...
            if len(poses) == 0:
                ...
                pos_codes = sorted(
                    blisscribe.POS_KEY, key=lambda k: blisscribe.POS_KEY[k]
                )
                poses = {pos_codes[idx] for idx in poses}

            lang = (
                sorted(blisscribe.BlissTranslator.supported_langs())[lang[0]]
                if len(lang) != 0
            ...
            synsets = self.translator.word_synsets(
                word, pos=poses, lang_code=self.translator.find_lang_code(lang)
            )
        ...
        self.bliss_label = Label(
            self.lookup_container,
            image=bliss_imgtk,
            width=bliss_img.size[0],
            height=bliss_img.size[1],
        )
